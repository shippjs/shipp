...
function makeRoutes(route, file, db, query) {
  ...
  Utils.eachFile(options.path, { sort : true }, function(file) {
    ...
    handler = function(req, res) {
      ...
      var context = _.clone(req.params);
          data    = _.extend({}, global.vars, makeDataQuery(db, metadata.data, context)());
    ...
    makeRoutes(options.url, file, db, metadata.query).forEach(function(slug) {
