...
module.exports = {
  ...
  **/
  ...
  add: function(ext, pipeline) {
    ...
    if (prev === pipeline) {
      console.log("   " + chalk.red("No Change:") + " shipp was already processing " + suffix(pipeline));
    } else {
      editor.set(key, pipeline);
      editor.save();
      if (prev && prev !== pipeline) console.log("   " + chalk.cyan("Removed:") + " shipp will no longer process " + suffix(prev));
      console.log("   " + chalk.cyan("Added:") + " shipp will process " + suffix(editor.get(key)));
    }
  ...
  **/
  ...
  remove: function(ext) {
    ...
    if (!prev) {
      ...
      editor.save();
      console.log("   " + chalk.cyan("Removed:") + " shipp will no longer process " + chalk.yellow("*." + ext) + " files with " + chalk.yellow(prev));
    }
